...
class States {
  getList() {
    return StatesWithCities.map(item => ({
      id: item.initials,
      value: item.name,
    }));
  }
  ...
  getStateByUf(uf) {
    return StatesWithCities.find(item => item.initials === uf);
  }

  getCitiesOfStateList(state) {
    const findedState = StatesWithCities.find(item => item.initials === state);
    const cities = findedState.cities.map(city => ({id: city, value: city}));
    return cities;
  }
}
